...
**Filtering creates a subset of the data based on criteria**. All records where the seats are greater than 14. All records where the department is "Journalism". **Filtering works with rows -- when we filter, we get fewer rows back than we start with.**

**Selecting returns only the fields named**. So if you only want to see department and course ID, you select those fields. When you look at your data again, you'll have two columns. If you try to use one of your columns that you had before you used select, you'll get an error. **Selecting works with columns. You will have the same number of records when you are done, but fewer columns of data to work with.**
...
## use == otherwise, it'll name everything "journalism"
...
selected_journalism_courses <- journalism_courses |> 
  select(id, title)
## if you do select(-id), that would show all the columns EXCEPT id
...
We don't have to run the operation over and over again. It's much cleaner when we're running many lines of code.
...
## you can also use the , if you don't like &
...
## the OR operator is | (shift_backslash)
## NOT EQUAL TO is != 

nrow(and_theatre_seats_15)
```

The or operator is additive, as noted below. That means that instead of showing classes that are both in the theatre department and 15 or more seats, it shows us classes that are in the theatre department *or* have 15 or more seats.
...
## str_detect means if you find Shakespeare ANYWHERE in the title, I want it - vs. if it includes Shakespeare but also includes something else in the title, it'll be listed as something else
...
## mutate says, take this column and do something with it
## column you want to change=date format, mdy (month/day/year)
## it says maryland_expenses twice because you first say this is what i'm working with, the second one tells it save it
...
```{r}
maryland_expenses |>
  mutate(month = floor_date(expenditure_date, "month")) |> ## this is a lubridate feature, will create new column
  group_by(month) |>
...
**It's formatted from just being a bunch of characters in R's mind to being actual numbers we can work with.**
...
**It's making variables for complicated calculations so the ultimate code is cleaner and creating new columns for these calculated values.**
...
**Arranged from greatest-least because of the arrange() function.**
...
**Sort of. It shows us the data in ascending order, so more useful if we want to know which counties voted for him the least.**
...
**str_equal figures out whether the two numbers/variables are equal, str_count counts the number of matches**
...
maryland_expenses_with_state <- maryland_expenses |>
  mutate(
    state = case_when(
        ...
        str_detect(address, " West Virginia ") ~ "WV", ## you have to do wv first because otherwise, virginia will overwrite virginia or the other way around <3
        str_detect(address, " Virginia ") ~ "VA",
        .default = NA
      )
    ## makes a NEW column with what state
  )
```
...
**Yes, after staring at it for a long time it makes sense. All in the theme of simplifiying code.**
...
1.  How many contributions came from PO boxes?

2.  How much money came to the Dundalk Eagle? From how many campaigns?

3.  How many contributions came from Baltimore?
