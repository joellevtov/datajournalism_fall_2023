...
not ordering it chronologically
...
Compared to the previous codeblock, what has changed and how did that affect the result? There are ~100 more classes now vs. last fall. The arrange function sorted it by count_classes by desc (descending). I don't have to put asc for ascending.
...
We have another column of data thanks to the "term, department"
...
Compared to the previous codeblock, what has changed and how did that affect the result? We have another column of data thanks to the "total_seats" row in the summarize codeblock.

### Task 13: different kinds of summarizations

**Task** Run the following codeblock to group the umd_courses dataframe by term. Then do the following summarizations for each term: 
- count the number of courses
- add up the total of all seats
- and calculate the mean and median contribution number of seats. Then sort so the term with the most total seats is listed first.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? We added more rows to the summarize function, and thus got more columns. This time, the columns are calculating more than just sums - now they're calculating medians and means. The top turns are all the fall semester, showing that we have significiantly more classes in fall than spring (nevermind winter or summer).
...
umd_courses |>
  ...
  arrange((total_seats), desc(count_classes))
```

Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted?

We have more columns - woo!! More significantly, we're able to see not only what department (from greatest-least) has the most seats available in total, we're able to see what department has 1. the largest number of seats, but 2. of those, which have the most classes.

### Task 12: Questions

No code here; I want you to provide three questions that this data suggests to you. What would you like to ask it?

1.  Are there years in which the top 10 departments with the greatest amount of seats have switched?

2.  What departments that are actually on campus (none of the VT or North Carolina crap) have the least number of seats available?

3.  How many classes has the Jewish Studies department had, and are the number of seats available this semester more or less than average?
