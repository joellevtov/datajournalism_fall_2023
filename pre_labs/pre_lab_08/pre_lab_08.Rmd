...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Rows: 3822 Columns: 4**
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **See below for answers**
...
baltco_911_calls_by_month <- baltco_911_calls |> #saving it out to a new dataframe
  ...
  group_by(month) |> #preparing the data for later on, in this case by organizing calls by month.
  summarize(total_calls = n()) |> #gives us the cute chart we're used to
  arrange(desc(total_calls)) #tells it how to arrange the data
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A big, empty square.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A bar graph shows us the number of calls per month.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **The months are no longer in chronological order - sorted by highest call volume.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Axes are flipped.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **We've made it a little prettier with a blue background.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **We've added a title, labels, and a source line.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A table like we're used to that's unorganized.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A graph that looks more like an earthquake seismonitor.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **This seems like a slightly more readable version of the above.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Honestly, I'm not sure what I'm looking at. It feels like a slightly more organized version of the first chart but I couldn't tell you why.**
