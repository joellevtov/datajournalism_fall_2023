...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **8 columns are in this dataset. The first column simply provides a reference number and is unimportant, ZIPCODE1 displays ZIP codes in the format normal people are used to (five digits), while ZIPCODE2 incorporates the five-digit ZIP code but adds the state code in front of it so we know what state the ZIP code is in. The ZIPNAME column gives us the name, and the rest of the columns are geometry math stuff we don't need to worry about (but the geometry column must not be cut).**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **The dataframe contains all the Maryland ZIP code and a name for all of them, some of which are more helpful than others.**
...
**Task** Run the following code. Describe the output in the space below. **The map generated shows every single Maryland zip code on top of a map of Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Er, nothing. It just downloads the package.**
...
**Task** Run the following code. Describe the output in the space below. **Besides the hilariously named Cuming county (I'm thinking of the spice, c'mon Derek), what the below does is generate a dataframe of every single county in the U.S with the FIPS code - that FIPS code is critical for literally doing anything using Census data to make maps. All 3,234 of them.**
...
This county shapefile has all 3233 U.S. counties. **Incorrect. It's actually 3,234 according to the tool. Merrill F.** We only want the Maryland counties, so we're going to filter the data to only keep Maryland counties. There is no STATE column, but there is a STATEFP column, with each number representing a state. Maryland's FP number is 24.
...
**Task** Run the following code. Describe the output in the space below. **This gives us a nice map of all the Maryland counties. I didn't realize this until literally two seconds ago but Baltimore City resembles the District of Columbia in terms of shape.**
...
**Task** Run the following code. Describe the output in the space below. **We get to see the land AND the sea.**
...
**Task** Run the following code. Describe the output in the space below. **This** **generates a table showing how many houses/properties were foreclosed upon in March 2023 in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **This shows us the number of foreclosures per ZIP code in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **This generates a map of Maryland with foreclosure data layered over top of it. The brighter a ZIP code, the more foreclosures happened there.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Er. Again, nothing. We just loaded another library.**
...
**Task** Run the following code. Describe the output in the space below. **The first chunk of code loads data provided by Maryland. This data includes all stages of foreclosure, from warnings to the actual foreclosure, before the data is sliced to only show the number of notices delivered that inform the owner that the bank intends to foreclose on the home. Then, it flips the axes so counties are rows again. Lastly, the code loads a new dataframe that contains the number of people in each county as of 2021.**
...
**Task** Run the following code. Describe the output in the space below. **The same table as above, except it includes geometry information now too.**
...
**Task** Run the following code. Describe the output in the space below. **This just shows us each dataframe.**
...
**Task** Run the following code. Describe the output in the space below. **This cleans the data as you explained above, making it join-able.**
...
**Task** Run the following code. Describe the output in the space below. **This finally shows us the population of each county, the number of people in each county and the geometry enabling us to plot it.**
...
**Task** Run the following code. Describe the output in the space below. **Same thing as above for the first two lines, then it enables us to have per capita data by dividing the number of notices by the population.**
...
**Task** Run the following code. Describe the output in the space below. **An ugly - but functional - map showing in visual terms the rate of foreclosures per county.**
...
**Task** Run the following code. Describe the output in the space below. **A little ugly but much easier to read, it shows us the hotspots for foreclosures.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **The poorer parts of Maryland to the south make up most of the foreclosures, wealthier counties like Montgomery have next to no foreclosures.**
