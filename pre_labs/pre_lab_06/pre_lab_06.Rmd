...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Not quite - the the UNA and OTH columns seem to switch every other year - musical chairs but for data?**
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **As Frederick has boomed in population, becoming another exurb of Washington (with almost no transit between the two), the number of registered voters has (logically) increased. But in 2016, Republicans outnumbered Democrats by roughly 50,000 voters. That equation was flipped on its head in 2020 - suddenly, Democrats outnumbered Republicans by roughly 40,000. The trend towards the Democratic party only grew between 2020 and 2022 - while the Republican party gained about 10,000 new registered voters, Democrats gained roughly 40,000. Interestingly, the Green Party - never large to begin with - became even smaller despite the population boom.**
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **There should be 24 rows for the 24 Maryland counties, and, indeed, out come 24 rows.**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **It contains county names, geocodes, and Maryland residents broken down by race and for some reason the number of people 18 or older in each county.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince George's, to no one's surprise, has the most Black voters over 18. The percent of a population that's Black correlates strongly with the percent of voters registered Democratic.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **There's a very rough correlation, maybe a .6, between the percent of the population that's over 18 and the percent of the population that's Asian. An interesting and important outlier, though, is Prince George's - despite having few Asian people over 18, it has a ton of Democrats.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Chr!**
