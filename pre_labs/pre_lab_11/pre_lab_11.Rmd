...
**Task** Run the following code and describe the dataframe it outputs. **It gathers the contents of Cardin's press release website in a table - one column for each section like the date and title.**
...
release_text <- tibble(url = character(), 
                       text = character())

# loop each url in the list of urls (each url is stored in "u")
for (u in urls){
...
releases <- releases |>
  mutate(
    text = 
      gsub("http.*","", text)) # this says look for anything that starts with "http" in the "text" column in releases
# the . is a character that can stand for anything
```
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **This function filters out the repeat words, but does not determine words to be the same regardless of capitalization.**
...
Fortunately, this is a solved problem with tidytext, which has a function called `unnest_tokens` that will convert the text to lowercase and remove all punctuation. 
The way that `unnest_tokens` works is that we tell it what we want to call the field we're creating with this breaking apart, then we tell it what we're breaking apart -- what field has all the text in it. For us, that's the `text` column:

### Task 6: Trying out unnest_tokens

**Task** Run the following code and describe the output of using the `unnest_tokens` function. **The code breaks apart sentences into individual words, thanks to the unnest_token "word" value. There's also a character value but I don't understand what it does differently than the word value. It produces different results but I don't understand how or why.**
...
unique_words <- releases |> 
  ...
  unnest_tokens(
    ...
    text)
View(unique_words)
# tokens = in linguist speech, "words," this tells it to pull apart sentences and search for every word

```

### Task 7: Make a column chart

**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Interesting? Eh. It shows that Cardin says "the" a lot. I'd be curious to transcribe interviews from various politicians just to see who says "uh" the most on average.**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **It's a hell of a lot more interesting -- all the filler words and articles are gone. Health turns out to be the most used word.**

```{r}
unique_words |>
  anti_join(stop_words) |> #ahh my lovely antijoin strikes again :) this says join everything BUT words in stop_word
  group_by(word) |>
  tally(sort=TRUE) |>
  mutate(percent = (n/sum(n))*100) |>
  top_n(50)

#don't pay too much attention the percents, they're all under 1%, but look at distance between percents
```
...
**Task** Run the following code and describe the results. **In 2023 (so far), Cardin suddenly started talking about "fire" and "women" as well as "american" a lot more. Women and american makes sense, but fire? Was he a firefighter in a previous life?**
...
**Task** Run the following code and describe the results. **Mixed bag. Some are super important and useful (and logical) - e.g. COVID-19. "Rescue plan" and "19 pandemic" are unehelpful - they just refer to the American Rescue Plan and the COVID-19 pandemic.**

```{r}
releases |>
  filter(date < '2022-01-01') |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |> #ngram = any combination of words in number, e.g. bigrams, trigrams
  separate(bigram, c("word1", "word2"), sep = " ") |>
  filter(!word1 %in% stop_words$word) |>
  filter(!word2 %in% stop_words$word) |> #filters ensure that it doesn't spit out "covid covid"
...
**Task** Run the following code and describe the results. **COVID-19 was still a topic, but much less so. This year seems to focus on building a lot more, interestingly enough. The only project I can think of off the top of my head are the new FBI headquarters, but that wouldn't explain baltimore. But in any case, a bit of a shift.**
...
**Task** Run the following code and describe the results. **Unsurprisingly enough, it shows us the most common two-word phrases for 2023 (so far). It's noteworthy here than COVID-19 has completely fallen off the deep end, it has been replaced with a lot of talk about the Dobbs Supreme Court case. But even more interestingly to me at least is that a phrase like "abortion rights" doesn't feature among them. Nor was abortion ever a word Cardin seemed to mention a lot. I suppose that's because the word is polarizing, framing it in terms of rights is a lot more paletable to all people.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Yes. But I find it much more interesting that at least in press releases Cardin didn't once mention the word "abortion." It makes sense as explained above but still suuper interesting. A fun, quirky one is "supreme." That's almost certainly referring to the Supreme Court, but Bing thinks "supreme" is positive. What a time it lives in... Question for you, though: Does bing let you do this analysis but for two- or three-word phrases?**
