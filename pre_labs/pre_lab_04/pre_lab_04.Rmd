...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Even in this small sample, I can see a ton of data rows that refer to the same thing but aren't quite identical. **
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **It transformed "Washington County Museum of Arts" to all uppercase and did the same to Charles County Arts Alliance.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Washington Adventist University was transformed to uppercase (though it wasn't smart enough to figure out that the above row refers to the same thing) **
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **No, not that I can tell. Scrolling through the first couple pages and comparing differences, I don't see much of anything to report.**
