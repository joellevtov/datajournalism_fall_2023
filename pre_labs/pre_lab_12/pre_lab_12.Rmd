...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **It is already done, it has all the information we want - the date and press release are in seperate columns.**
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **It would be helpful to have a column that scrapes the hyperlink on the website and puts the link into another column.**
...
tags there are in the HTML source? **Each li is nested inside a ul tag (which is pretty funny). There are several ul tags as one nears the end of the page besides the press release table ul.**
...
tags. Describe the output: **This gives us in plain text what it has scraped from the website, i.e. junk including the social headers but also the plain text of the press release titles.**
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **This makes a dataframe, finally, out of the above data. It's still formatted in the same way: One column that contains all the info, including the junk.**
