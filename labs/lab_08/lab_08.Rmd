...
pg_overdoses_2022 <- pg_overdoses_2022 |> 
  as_datetime(date = date(datetime)) |> 
...
```{r}
month_pg_overdoses_2022 <- pg_overdoses_2022 |> 
  group_by(month) |> 
  summarize(count=n())

count_month_pg_overdoses_2022 <- month_pg_overdoses_2022 |> 
  summarize(sum(count))

month_pg_overdoses_2022 <- month_pg_overdoses_2022 |> 
  mutate(
    percent_of_calls = ((count/1397)*100)
  )

month_pg_overdoses_2022 |> 
  arrange(desc(percent_of_calls)) |> 
  ggplot() +
  geom_bar(aes(x=reorder(month, percent_of_calls), weight=percent_of_calls)) +
  coord_flip() +
  labs(
    title="Most 911 calls happen in the last half of the the year",
    x = "month",
...
```{r}
week_pg_overdoses_2022 <- pg_overdoses_2022 |> 
  group_by(week) |> 
  summarize(count=n())

count_week_pg_overdoses_2022 <- week_pg_overdoses_2022 |> 
  summarize(sum(count))

week_pg_overdoses_2022 <- week_pg_overdoses_2022 |> 
  mutate(
    percent_of_calls = ((count/1397)*100)
  )

one_week_pg_overdoses_2022 <- pg_overdoses_2022 |> 
  group_by(date) |>
  summarise(
    total_calls=n()
  )
...
week_pg_overdoses_2022 |> 
  ggplot() +
  geom_line(aes(x=week, y=count)) +
  scale_x_continuous(breaks = scales::pretty_breaks(n=10)) +
  labs(
    title="911 Overdose Calls Peak around July",
    x = "month",
    y = "number of calls",
...
## silly little nonworking except could probably work if i cared enough to research it
## one_week_pg_overdoses_2022 |> 
##  ggplot() + 
##  geom_line(aes(x=date, y=total_calls)) +
...
```{r}
county_pg_overdoses_2022 <- pg_overdoses_2022 |> 
  group_by(zipcode) |> 
...
county_pg_overdoses_2022 |> 
  inner_join(acs, join_by(zipcode==GEOID)) |> 
  ggplot() +
  geom_point(aes(x=total_calls, y=estimate))+
  labs(
    title="Poorer communities have more 911 overdose calls",
    x = "number of calls",
    y = "income",
    caption = "source: Prince George's County EMS"
      )
