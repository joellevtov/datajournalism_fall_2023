...
## new column to see if this works
add_column (
  .cleaned_maryland_expenses
)
...
```{r}
cleaned_maryland_expenses
no_na_media_expenses <- cleaned_maryland_expenses %>% 
  filter(!is.na(payee_name)) %>% 
  filter(expense_category=="Media")

no_na_media_expenses
write_csv(no_na_media_expenses, "data/maryland_expenses_without_na.csv")
```
