...
Start Tabula, then go to <ƒ> in your browser. Click the "Browse" button and find the PDF file and click "open", and then click the "Import button" in Tabula. This will take a few seconds or longer.
...
A1. July 14, 2022 and October 10, 2022 had the most amount of calls on one day - 23. No, there are no days with no calls, according to the data. The least amount of calls were on June 11 and February 6, with one call each.
...
A2. Saturday is the most popular day for calls. Roughly 16% of calls were on that day.
...
A3. According to R's count, 4540 Silver Spring Rd, what looks to be a multi-family house or a large single-family one, has the most calls. But among the highest ranking are two addressed with a "PC" in front of them, which makes me wonder a) what that refers to b) whether some addresses are listed once normally once with a "PC" in front - that would, of course, change the results.
...
A4. Why are individual houses getting more calls than motels, which we stereotypically associate with these drug overdose problems.
